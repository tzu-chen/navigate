...
│   │   ├── App.tsx               # Root component, 8 view modes, global state
...
* **App.tsx** — Root component managing 8 view modes: `browse`, `library`, `authors`, `viewer`, `chatHistory`, `comments`, `worldline`, `walkthroughs`. Holds global state for papers, tags, and favorite authors. Initializes color scheme and font size from localStorage on mount.
* **components/** — 21 components:
  ...
  + `WalkthroughGallery` — The `walkthroughs` tab: every built walkthrough as a wall of covers (see Walkthrough Mode)
  + `WalkthroughCover` — The generated card graphic; deterministic per paper, motif drawn from the outline's visual kinds
...
* **routes/** — RESTful route handlers:
  ...
  + `walkthrough.ts` — generated interactive explainers: source manifest, outline, agentic build (202 + SSE), sandboxed bundle serving, vendored assets, and the gallery listing. See Walkthrough Mode below.
...
⚠️ **And it must be the `-full` TeX build (`es5/tex-svg-full.js`), for the same reason one level down.** Plain `tex-svg.js` pre-loads only seven TeX packages — base, ams, newcommand, noundefined, require, autoload, configmacros — and leaves every other extension to **`autoload`, which fetches it at _typeset_ time** from `loader.paths.mathjax`. The asset route serves exactly two filenames, so the first `\boldsymbol` in a paper (and ML papers are full of them) went to `/api/walkthrough/asset/input/tex/extensions/boldsymbol.js`, 404'd, rejected the loader promise, rejected `typesetPromise` with it — and **every equation in that scene stayed raw, not just the one that used the macro**. Measured on `2606.05878` (TS-ICL): the whole opening scene rendered as literal TeX while the canvas figure beside it drew perfectly, which is what made it look like a content bug rather than a loading one. `tex-svg-full.js` pre-loads every extension (`boldsymbol`, `cancel`, `color`, `braket`, `physics`, `mhchem`, `unicode`, …) so autoload resolves from the in-memory registry and never touches the network; it costs **166 KB** more (2.28 MB vs 2.11 MB), which is nothing against being unable to fail. The rule generalizes: **a vendored library that can lazily fetch a _part_ of itself is not vendored**, and inside the sandbox that failure is silent and total rather than degraded.

The **served asset name deliberately did not change**. It is unversioned and sent `no-cache`, so swapping the file behind it repaired every walkthrough already built with no bundle migration at all — the payoff of the `immutable`-is-unsafe decision above, collected.

⚠️ **A `typesetPromise` rejection must not be swallowed.** It rejects for the *whole element*, so the failure above cost an entire scene while `.catch(function () { /* leave the source visible */ })` logged nothing anywhere — which is how it survived seven builds and was found only from a screenshot. `WT.typeset` now reports through `WT.error` (the app shows it as a notification) and then **retries block by block**, so a future unresolvable macro costs one equation instead of the page. `verify:walkthrough` reads the served filename off the route's exported `ASSETS` map — it was typed in both places, so the gate vouched for a file the route no longer served — and asserts the served build's own `preLoad(...)` list contains the extensions a physics or ML paper actually reaches for.
...
**Narrow `postMessage` protocol**, validated on both ends. iframe → app: `ready`, `error`, `gotoPage`, `key`. app → iframe: `theme`, `setScene`. Nothing else is honored. The frame is in an opaque origin so `event.origin` is `"null"`; identity is established by comparing `event.source` to the frame's `contentWindow`. CSS custom properties do not cross an iframe boundary, so the app **sends** its resolved `--mono-*` palette rather than relying on inheritance, and re-sends it when the theme changes.

⚠️ **`key` exists because focus is one-way.** The app binds its shortcuts on its own `window`, and a click anywhere in the bundle moves focus into the iframe — after which every keystroke is delivered to *that* document and the app never sees it. So the pane-layout keys, whose whole job is getting you back out of the walkthrough, stopped working exactly once you were using one. The opaque origin means neither side can read the other's events, so the frame forwards them explicitly and `WalkthroughPane` re-dispatches a synthetic `keydown` on `window`. Re-dispatching rather than calling the pane handlers directly is what makes *every* viewer shortcut work from inside the frame — tier digits, immersive, panel, save — instead of only the three the bug was reported for. Three guards on the sending side, re-checked on receipt because a sandboxed sender's guarantees are its own: **`isTrusted`** (true only for a real keystroke, so generated code cannot `dispatchEvent` its way into pressing keys in the host app — the frame is sandboxed precisely because a model wrote its contents from third-party text), unmodified **single-character keys only** (the shape the app binds; Arrow/Tab/Escape stay with the scene stepper), and nothing typed into a field. `PaperViewer` also blurs a focused iframe when the walkthrough is hidden, so the keyboard can never be left pointing at a frame that no longer receives anything.
...
**UI.** `PaperViewer` gets a three-way pane toggle — **PDF only / split / walkthrough only** — not a new top-level `ViewMode`, so the whole sidebar (chat, comments, worldline) stays live beside the walkthrough. Split is the mode that makes a walkthrough genuinely useful: the point of an explainer is checking it against the paper. Each layout also has a keybinding (`[` / `\` / `]` by default, rebindable in Settings → Shortcuts, scope **Pane layout**), because flipping between the paper and its explainer is what the toggle is *for* and it happens often enough that reaching for the pointer each time is the wrong cost; the bracket keys are spatial (left pane / both / right pane) and stay clear of the tier digits and the single-letter viewer actions. `PaneMode` therefore lives in `types.ts` rather than `PaperViewer`, so the gallery can name the pane a paper opens into.
...
**The outline does not disappear once a bundle exists.** It used to: the moment a build landed, the pane swapped the scene plan for the iframe, which is exactly when *"what was this scene supposed to show?"* starts being worth asking. A header toggle (Outline ⇄ Visualization) now calls it back on a built row. It renders as an **overlay** rather than swapping the iframe out — hiding the frame would give it a zero-size box, and the MathJax hazard above is a hazard whenever that happens, not only at first mount — so the frame keeps its box, its scroll position and its current scene underneath. Both share a positioned `.wtp-body` wrapper; that wrapper exists for this and nothing else. The overlay keeps *Edit outline* (which forks a new row, so the paid-for build survives) and *Re-outline*, but not *Build*: the header's Rebuild is the button for that, and a second "Build" beside a finished walkthrough reads like it would produce something new when it would in fact hit the cache and hand back the same bundle.

**The `walkthroughs` tab is the index of everything you have built** (`WalkthroughGallery`, `GET /api/walkthrough/gallery`). A walkthrough lived one pane deep inside one paper, so finding one again meant remembering which paper it belonged to. It is a *gallery* rather than a list because what it indexes is visual: three cards to a row, each a large cover plus the paper's title, with search, a built/outline-only filter, and sorting by date, title, scene count or cost. Clicking one opens the paper straight into `paneMode: 'walkthrough'`.

The route collapses to **one entry per paper**, not per row — multiple rows per paper are expected (an outline edit forks one) and a wall of near-duplicates is not an index — taking the newest `ready` row and falling back to the newest row of any status, with `buildCount` for the rest, which the pane's own history menu reaches. Because a walkthrough outlives its paper, an entry can be marked "not in library" and still open: `App` synthesizes an `ArxivPaper` from the entry, shows it at once, and upgrades it with the real arXiv record when that answers.

**The covers are generated, not screenshots** (`WalkthroughCover`). A bundle is a live document that typesets MathJax and may run WebGL, so thumbnailing a wall of them would mean a wall of iframes — megabytes and seconds each, for a picture. What the gallery has instead is the outline, which says what every scene lets you manipulate, and that is enough to draw from: the motif comes from the walkthrough's dominant `visual.kind` (`field` really is a vector field, `graph` really is a node-link sketch, an all-`none` outline gets a reading-shaped prose motif), the density from the scene count, and every random choice from an FNV-1a hash of the arXiv id through a mulberry32 PRNG. Deterministic, so a paper's cover never changes under the reader; distinct, so the wall is scannable; and honest, in that two covers look alike exactly when the two walkthroughs are alike. Colours are `--mono-cat-*` custom properties rather than literals, so the covers follow the theme for free.

⚠️ **The title has to come off the walkthrough row.** Neither `papers` nor `paper_archive` is a reliable source: a walkthrough outlives its paper *and* can be outlined straight from a browse listing for a paper that was never saved at all, which left the gallery printing a bare arXiv id. `walkthroughs.paper_title` records what the outline pass already resolved; the gallery still prefers the live tables (fresher), then the archive, then this. `initializeDatabase()` backfills it once from what the local tables know — a row whose paper was never saved anywhere stays null and shows as its id, which is at least its true identity.
...
| `walkthroughs` | id, arxiv_id, source_version, source_sha, contract_version, cache_key, status, fitness, **paper_title**, outline, bundle_path, warnings, model, backend, token counts, outline_cost, build_cost, estimated_cost, error | cache_key UNIQUE, status CHECK ('pending','building','ready','failed','unfit'), **no FK — it outlives the paper row** |
